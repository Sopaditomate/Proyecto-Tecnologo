...
class OrderControllerAdmin {
    ...
    async getActiveOrders(req, res) {
        try {
            ...
            res.status(500).json({ message: 'Error al obtener órdenes activas', error: error.message });
        }
    }
    ...
    async getAllOrdersAndDetailsByUserId(req, res) {
        ...
        try {
            const orders = await OrderModelAdmin.getAllOrdersAndDetailsByUserId(userId);
            res.status(200).json(orders);
        } catch (error) {
            res.status(500).json({ message: 'Error al obtener órdenes por ID de usuario', error: error.message });
        }
    }
    ...
    async getOrderDetailsByOrderId(req, res) {
        ...
        try {
            const orderDetails = await OrderModelAdmin.getOrderDetailsByOrderId(orderId);
    ...
    async updateOrderStatus(req, res) {
        ...
        try {
            const success = await OrderModelAdmin.updateOrderStatus(orderId, statusId);
            if (success) {
    ...
    async getOrderStatus(req, res) {
        try {
            ...
            res.status(500).json({ message: 'Error al obtener estados de órdenes', error: error.message });
        }
    }
}
