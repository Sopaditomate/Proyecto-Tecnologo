...
class RecetasModel {
  ...
  async UpdateReceta(id, ID_PRODUCTO, ID_MATERIA, CANTIDAD_USAR) {
    const [producto] = await pool.query(`UPDATE recetas SET ID_PRODUCTO = ?, ID_MATERIA = ?, CANTIDAD_USAR = ? WHERE ID_RECETA = ?`,
      [ID_PRODUCTO, ID_MATERIA, CANTIDAD_USAR, id]);
