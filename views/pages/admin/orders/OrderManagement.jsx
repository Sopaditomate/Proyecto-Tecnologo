...
export const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const response = await axios.get("http://localhost:5001/api/pedidos/active-orders");
  ...
  const fetchOrderStatuses = async () => {
    try {
      const response = await axios.get("http://localhost:5001/api/pedidos/order-status");
      if (Array.isArray(response.data)) {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      const response = await axios.put("http://localhost:5001/api/pedidos/orders/update-status", {
